...
#Data Visualization
#Visual 1
visual1 <- ggplot(data, aes(Age, colour = Outcome)) +
  geom_freqpoly(binwidth = 1) + labs(title="Age Distribution by Outcome")

#visual 2
visual2 <- ggplot(data, aes(x=Pregnancies, fill=Outcome, color=Outcome)) +
  geom_histogram(binwidth = 1) + labs(title="Pregnancy Distribution by Outcome")
visual2 <- visual2 + theme_bw()

#visual 3
visual3 <- ggplot(data, aes(x=BMI, fill=Outcome, color=Outcome)) +
  geom_histogram(binwidth = 1) + labs(title="BMI Distribution by Outcome")
visual3 <- visual3 + theme_bw()

#visual 4
visual4 <- ggplot(data, aes(Glucose, colour = Outcome)) +
